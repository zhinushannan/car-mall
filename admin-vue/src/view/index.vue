<template>

  <div>

    <!-- 左边 导航栏 -->
    <div class="left-side" style="background-color: #545c64">
      <!-- 顶部盒子 显示系统名称 头像 欢迎 -->
      <div class="left-top-box">
        <div class="left-top-title" style="color: #ffd04b">{{ adminName }}</div>
        <div class="left-top-avatar">
          <!--          <img src="./img/head.jpg" />-->
        </div>
        <div class="left-top-welcome" style="color: #ffd04b">{{ adminName }}欢迎你！</div>
      </div>
      <!-- 导航栏盒子-->
      <div class="left-navs-box">
        <div class="navs-inner-box">
          <el-menu :default-active="id" class="navs-item-box" background-color="#545c64" text-color="#ffffff"
                   active-text-color="#ffd04b">
            <el-menu-item @click="welTab" index="0">
              <i class="el-icon-s-platform"></i>
              <span slot="title">控制台</span>
            </el-menu-item>
            <div v-for="(item) in navs" :key="item.id">
              <!-- 没有子选项 -->
              <el-menu-item @click="openTabs(item)" v-if="item.href!=='#'" :index="item.id">
                <i :class="item.icon"></i>
                <span slot="title">{{ item.title }}</span>
              </el-menu-item>
              <!-- 有子选项 -->
              <el-submenu v-if="item.href==='#'" :index="item.id">
                <template slot="title"><i :class="item.icon"></i>{{ item.title }}</template>
                <el-menu-item v-for="(item) in item.children" @click="openTabs(item)" :key="item.id" :index="item.id">
                  <i :class="item.icon"></i>
                  {{ item.title }}
                </el-menu-item>
              </el-submenu>
            </div>
          </el-menu>
        </div>
      </div>
    </div>
    <!-- /左边 导航栏 -->

    <!-- 右边区域 -->
    <div class="right-box">
      <!-- 顶部栏 -->
      <div class="right-header">
        <i @click="isShowNav" :class="showNav ? 'el-icon-s-fold':'el-icon-s-unfold'"></i>
        <div class="header-right">
          <el-avatar size="small" src="./img/head.jpg"></el-avatar>
          <!--个人信息操作 -->
          <el-dropdown :show-timeout=100 @command="personCommand">
							<span class="el-dropdown-link">
								{{ adminName }}<i class="el-icon-arrow-down el-icon--right"></i>
							</span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="center">个人中心</el-dropdown-item>
              <el-dropdown-item command="base">基本资料</el-dropdown-item>
              <el-dropdown-item command="logout" divided>退出登录</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>

      </div>

      <!-- tab 标签栏 -->
      <div class="right-tab-box">
        <div class="tab-item-box">
          <div class="tab-item-ul">
            <div class="tab-item" @click="welTab" :class="welcome?'tab-item-active':'1'">
              <i class="el-icon-s-platform  tab-item-icon"></i>
              <div class="tab-item-text">控制台</div>
            </div>
            <div class="tab-item" @click="tabActive(item)" :id="item.id" :class="id===item.id?'tab-item-active':'1'"
                 v-for="(item,index) in tabs">
              <i class="tab-item-icon" :class="item.data.icon"></i>
              <div class="tab-item-text">{{ item.data.title }}</div>
              <i @click.stop="tabClose(item)" class="el-icon-close tab-item-close"></i>
            </div>
          </div>
        </div>
        <div class="tabs-close-box">
          <el-dropdown @command="tabsCloseItem" :show-timeout=100>
            <i class="el-icon-arrow-down"></i>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="a">关闭当前标签页</el-dropdown-item>
              <el-dropdown-item command="b">关闭其他标签页</el-dropdown-item>
              <el-dropdown-item command="c">关闭所有标签页</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>
      </div>

      <!-- 内容区域 iframe -->
      <div class="right-contain">
        <template v-for="(item) in tabs">
          <iframe :src="item.data.href" width="100%" height="100%" frameborder="none" v-show="id===item.id">
          </iframe>
        </template>

      </div>

    </div>


  </div>


</template>

<script>
export default {
  name: "Index",
  data() {
    return {
      activeIndex: '0',
      adminName: 'admin',
      navs: [
        {
          "title": "用户管理",
          "icon": "el-icon-user-solid",
          "href": "#",
          id: "1",
          children: [
            {
              "title": "用户列表",
              "icon": "el-icon-user",
              "href": "/#/user/list",
              "id": "1-1"
            },
          ]
        },
        {
          "title": "汽车管理",
          "icon": "el-icon-user-solid",
          "href": "#",
          "id": "2",
          children: [
            {
              "title": "汽车列表",
              "icon": "",
              "href": "/#/car/list",
              "id": "2-1"
            }
          ]
        },

      ],

      isCollapse: true,
      welcome: true,
      showNav: true,
      tabs: [],
      id: '0',
      i: 0,
      isActive: 'tab-item-active',
      loadOk: false,
      item: '',
    }
  },
  methods: {
    isShowNav(e) { //隐藏显示左边导航栏
      if (this.showNav) {
        $(".left-side").animate({left: '-220px'}, "slow");
        $(".right-box").animate({left: '0'}, "slow");
        this.showNav = !this.showNav
      } else {
        $(".left-side").animate({left: '0px'}, "slow");
        $(".right-box").animate({left: '220px'}, "slow");
        this.showNav = !this.showNav
      }
    },
    welTab() {
      this.welcome = true
      this.id = '0'
      this.activeIndex = '0'
      $('.tab-item-ul').css("left", 0)
    },
    openTabs(e) {
      this.welcome = false
      if (this.tabs.length === 0) {
        this.tabs.push({id: e.id, data: e})
        this.id = e.id
        this.item = e
        this.calWidth()
      } else {
        let back = this.checkTab(e.id)
        if (back === 1) { //可以插入
          this.tabs.push({id: e.id, data: e})
          this.id = e.id
          this.item = e
          this.calWidth()
        } else { //已打开
          this.id = e.id;
          this.item = e
          this.calTabWid(e.id)
        }
      }
    },
    calWidth() {
      let tbox = $('.tab-item-box').width() //外边盒子距离长度
      let titem = $('.tab-item')
      let len = this.tabs.length + 2 //有几个tabs
      let ul = $('.tab-item-ul').width() + 100
      if (tbox - ul < 0) {
        let len = tbox - ul
        $('.tab-item-ul').css("left", len - 20)
      }
    },
    checkTab(id) { //检查tabs是否已经打开
      let i = 0;
      let back = 0
      let tabs = this.tabs
      for (i; i < tabs.length; i++) {
        if (tabs[i].id === id) {
          back = 0
          break //找到id 中断循环
        } else {
          back = 1
        }
      }
      return back
    },
    calTabWid(id) {
      // console.log(id)
      let tbox = $('.tab-item-box').width() //外边盒子距离长度
      let ul = $('.tab-item-ul').width() //tabs外边盒子长度
      let left = $('.tab-item-ul').position().left; //ul 左边偏移距离
      let titem = $('#' + id)
      if (titem.length > 0) {
        let iLeft = titem.position().left //tab距离父盒子左边距离
        let titemW = $('#' + id).width()
        if (ul > tbox && left <= 0) {
          let disAreaL = -left //可视区域左边
          let disAreaR = -left + tbox //可视区域右边
          let iRight = titemW + iLeft
          if (iLeft >= disAreaL && iRight <= disAreaR) {
            // console.log('不需要移动')
          } else {
            if (iLeft < disAreaL) {
              $('.tab-item-ul').css("left", left + (disAreaL - iLeft) + 50)
            } else if (iRight > disAreaR) {
              $('.tab-item-ul').css("left", left + (disAreaR - iRight) - 20)
            }
          }
        }
      }
    },
    // 个人信息操作
    personCommand(e) {
      switch (e) {
        case 'center':
          this.$message("个人中心");
          break;
        case 'base':
          this.$message("基本资料");
          break;
        case 'logout':
          this.$confirm('确定退出登录吗?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            window.location.href = "./pages/login/login.html"
          }).catch(() => {
            this.$message({type: 'info', message: '已取消退出'});
          });
          break;
      }
    },
    tabActive(e) {
      this.welcome = false
      this.id = e.id;
      this.item = e.data
      this.activeIndex = e.id
      this.calTabWid(e.id)
    },
    tabClose(e) {
      this.closeTabThis(e.id)
    },
    closeTabThis(id) {
      let tabs = this.tabs
      let i = 0;
      let len = this.tabs.length - 2
      if (id === 0) {
        layer.msg('这个不能关闭的哦')
        return
      }
      for (i; i < tabs.length; i++) { //找到id在tabs数组中的位置 下标
        // console.log(tabs[i].id)
        if (tabs[i].id === id) {
          break;
        }
      }
      // console.log(i)
      this.tabs.splice(i, 1) //从tabs中移除

      if (tabs.length === 0) { //如果tabs没有数据，将控制台设置变色
        this.welcome = true
        this.id = '0'
      } else {
        if (tabs[len].id !== this.id) { //判断tabs数组中最后一组数据的id是否为当前的id，若不是则设置
          this.id = tabs[len].id
          this.item = tabs[len].data
        }
      }
      //调整位置
      this.closeWidth()
    },
    closeWidth() {
      let tbox = $('.tab-item-box').width() //外边盒子距离长度
      let ul = $('.tab-item-ul').width() //tabs外边盒子长度
      let left = $('.tab-item-ul').position().left; //ul 左边偏移距离
      if (ul > tbox) {
        $('.tab-item-ul').css("left", tbox - ul + 60)
      } else {
        $('.tab-item-ul').css("left", 0)
      }
    },
    tabsCloseItem(e) {
      if (e === 'a') {
        // layer.msg('关闭当前')
        this.closeTabThis(this.id)
      } else if (e === 'b') {
        // layer.msg('关闭其他')
        this.closeTabOther()
      } else {
        // layer.msg('关闭所有')
        this.closeTabAll()
      }
    },
    closeTabOther() {
      if (this.tabs.length === 0) {
        return
      } else {
        if (this.id === 0) {
          this.tabs = []
        } else {
          let item = this.item
          this.tabs = []
          this.tabs.push({id: item.id, data: item})
          $('.tab-item-ul').css("left", 0)
        }
      }
    },
    closeTabAll() {
      this.tabs = []
      this.welcome = true
      this.id = '0'
      $('.tab-item-ul').css("left", 0)
    }
  }
}
</script>

<style scoped>
html, body {
  padding: 0;
  margin: 0;
  width: 100%;
  height: 100%;
}

#app {
  width: 100%;
  height: 100%;
  display: flex;
}

/* 首页采取左右盒子布局  左边只放导航栏 右边盒子放顶部栏 内容盒子 底部栏*/

/* 左边导航栏 */

.left-side {
  width: 220px;
  /* height: 100%; */
  /* background-color: #545c64; */
  position: absolute;
  top: 0;
  left: 0;
  bottom: 0;
}

/* 左上盒子 */

.left-top-box {
  width: 100%;
  height: 200px;
  /* background-color: #545c64; */
  display: flex;
  flex-direction: column;
  align-items: center;
  position: relative;
  /* justify-content: center; */
}

/* 系统名称 */

.left-top-title {
  width: 100%;
  /* color: #ffd04b; */
  font-size: 20px;
  border-bottom: 1px solid #646464;
  display: flex;
  justify-content: center;
  align-items: center;
  height: 50px;
}

/* 用户头像 */

.left-top-avatar {
  width: 90px;
  height: 90px;
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  background-color: #7b7256;
  margin-top: 20px;
}

.left-top-avatar img {
  width: 80px;
  height: 80px;
  border-radius: 50%;
}

.left-top-welcome {
  /* color: #ffd04b; */
  font-size: 12px;
  margin-top: 10px;
}

/* 左边导航栏 */

.left-navs-box {
  width: 220px;
  bottom: 0;
  left: 0;
  top: 200px;
  position: absolute;
  overflow: hidden;
}

.navs-item-box {
  width: 220px;
  /* height: 500px; */
}

.navs-inner-box {
  position: absolute;
  overflow-y: scroll;
  top: 0;
  left: 0;
  bottom: 20px;
}

/* 右边区域 */

.right-box {
  height: 100%;
  background-color: #EEEEEE;
  position: absolute;
  left: 220px;
  top: 0;
  right: 0;
}

/* 顶部栏 */

.right-header {
  width: 100%;
  height: 50px;
  background-color: #FFFFFF;
  display: flex;
  align-items: center;
  justify-content: space-between;
}

.right-header i {
  font-size: 28px;
  margin-left: 10px;
}

/* 顶部栏右边 */

.header-right {
  /* width: 150px; */
  display: flex;
  /* justify-content: space-between; */
  align-items: center;
  margin-right: 20px;
}

.item {
  margin-right: 40px;
}

.el-dropdown-link {
  margin-left: 10px;
}

.header-right i {
  font-size: 18px;
}

/* tab 栏 */

.right-tab-box {
  /* width: 100%; */
  height: 40px;
  background-color: #F5F7F9;
  display: flex;
  position: relative;
  max-width: 100%;
  align-items: center;
}

/* 左边箭头 */

.tab-left {
  width: 40px;
  height: 40px;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 18px;
  position: absolute;
  left: 0px;
  /* background-color: #00FF00; */
  z-index: 10;
}

.tab-right {
  width: 40px;
  height: 40px;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 18px;
  position: absolute;
  right: 40px;
  /* background-color: #00FF00; */
  z-index: 10;
}

/* 关闭tabs 组 */

.tabs-close-box {
  width: 35px;
  height: 30px;
  margin-right: 10px;
  background-color: white;
  display: flex;
  align-items: center;
  border-radius: 5px;
  justify-content: center;
}

.tab-left a:hover {
  color: #007DDB;
}

.tab-right a:hover {
  color: #007DDB;
}

.tab-item-box {
  width: 100%;
  height: 50px;
  z-index: 9;
  margin: 0 40px;
  display: flex;
  align-items: center;
  overflow: hidden;
  position: relative;
  z-index: 9;
  padding: 0 5px;
  -webkit-transition: all .5s;
}

/* tabs 标签列表盒子 可隐藏标签 */

.tab-item-ul {
  min-width: 200px;
  z-index: 11;
  position: absolute;
  display: flex;
  justify-content: space-between;
  /* background-color: #007DDB; */
  left: 0px;
  transition: all .5s;
  -webkit-transition: all .5s;
  width: max-content;
}

.tab-item {
  width: auto;
  height: 30px;
  background-color: #FFFFFF;
  border-radius: 3px;
  display: flex;
  align-items: center;
  padding: 0 10px;
  margin: 0 5px;
  position: relative;
  cursor: pointer;
}

.tab-item-active {
  color: #01AAED;
}

.tab-item-icon {
  margin-right: 5px;
}

.tab-item-text {
  /* width: auto; */
  min-width: 45px;
  font-size: 14px;
  text-align: center;
  display: flex;
  justify-content: center;
  align-items: center;
  flex: 1;
}

.tab-item-close {
  width: 20px;
  height: 20px;
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 1999;
}

.tab-item-close:hover {
  background-color: #ffad7d;
  color: white;
  border-radius: 50%;
  z-index: 1999;
}

/* 内容页面区域 */

.right-contain {
  /* width: 96%;
  height: 80vh; */
  background-color: #f5f5f5;
  /* padding: 0 2%; */
  position: absolute;
  right: 10px;
  left: 10px;
  top: 100px;
  bottom: 10px;
}

/* 底部栏 */

.right-footer {
  position: absolute;
  bottom: 0;
  height: 50px;
  left: 0;
  right: 0;
  color: white;
  display: flex;
  align-items: center;
  border-left: 1px solid #fff;
}

.right-footer span {
  margin-left: 100px;
}

/* 覆盖样式 */

.el-menu {
  border-right: solid 0px #e6e6e6 !important;
}

/* 公告 */

.notice-box {
  text-align: center;
}
</style>
